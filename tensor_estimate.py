...
class vrs_prediction:
    def __init__(self, tensor_shape, dim, M, X_train, N_train):
        ...
        self.total_points = len(X_train)
        # empirical mean number of points per realization (Poisson mean Λ̂)
        if self.N_train <= 0:
            raise ValueError("N_train must be positive.")
        self.mean_count = self.total_points / self.N_train
    ...
    # -------- Spatial Poisson process sampling (one realization) --------
    def sample_poisson_spp(self, random_state=None):
        """
        Sample ONE realization of a spatial Poisson process on the original
        domain with intensity λ(x) estimated by A_predict.

        Steps:
          1) N ~ Poisson(Λ̂), where Λ̂ = total_points / N_train
          2) Given N, sample N i.i.d. points from the normalized density
             proportional to λ(x) using sampling_N
          3) Map points back to original domain via self.new_domain

        Returns:
            X_samples: array of shape (N, dim) in the ORIGINAL domain.
                       If N=0, shape is (0, dim).
            N:         integer, number of points in this realization.
        """
        rng = np.random.default_rng(random_state)

        # Poisson mean Λ̂ estimated from training data
        lam_hat = self.mean_count

        # 1) Sample N from Poisson(Λ̂)
        N = rng.poisson(lam_hat)

        if N == 0:
            # No points in this realization
            return np.empty((0, self.dim), dtype=float), 0

        # 2) Sample N points in [0,1]^dim from normalized density ∝ λ(x)
        Z_samples = self.sampling_N(N)

        # 3) Map back to original domain
        X_samples = self.new_domain.inverse_compute_data(Z_samples)
        return X_samples, int(N)

    # -------- Spatial Poisson process sampling (multiple realizations) --------
    def sample_poisson_spp_batch(self, n_realizations, random_state=None):
        """
        Sample n_realizations independent realizations of the spatial Poisson process.

        For i = 1..n_realizations:
            N_i ~ Poisson(Λ̂)
            X^{(i)}_1,...,X^{(i)}_{N_i} i.i.d. ~ normalized density ∝ λ(x)

        Returns:
            points_all: array of shape (sum_i N_i, dim) in ORIGINAL domain.
                        If sum_i N_i == 0, shape is (0, dim).
            Ns:         array of shape (n_realizations,), the Poisson counts N_i.
        ...
        lam_hat = self.mean_count
        Ns = rng.poisson(lam_hat, size=n_realizations)
        total_N = int(Ns.sum())

        if total_N == 0:
            # No points at all across all realizations
            return np.empty((0, self.dim), dtype=float), Ns

        points_all = np.empty((total_N, self.dim), dtype=float)
        ...
        for N in Ns:
            ...
            # sample N points in [0,1]^dim from normalized density
            Z_samples = self.sampling_N(int(N))
            # map to original domain
            X_samples = self.new_domain.inverse_compute_data(Z_samples)

            points_all[idx:idx + N, :] = X_samples
            idx += N

        elapsed = time.perf_counter() - start
        return points_all, Ns, elapsed
